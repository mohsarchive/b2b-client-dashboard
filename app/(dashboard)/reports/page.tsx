...
export default function ReportsPage() {
  ...
  const download = (reportName: string) => {
    const csv = [['Metric', 'Value', 'Change'], ...metrics.map((m) => [m.label, m.value, `${m.delta}%`])].map((r) => r.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(',')).join('\n')
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }))
    const a = document.createElement('a')
    a.href = url
    a.download = `${reportName.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'report'}.csv`
