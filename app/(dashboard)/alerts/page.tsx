...
import { ArrowLeft, BellRing, Check, Plus, Trash2, Zap } from 'lucide-react'
...
export default function AlertsPage() {
  ...
  const add = () => { if (!name.trim()) return; save([{ id: `rule-${Date.now()}`, name: name.trim(), event, enabled: true }, ...rules]); setName(''); setShowNew(false) }
  return <div className="space-y-6">
    ...
    <section className="grid gap-3 md:grid-cols-2">
      {rules.map((rule) => <div key={rule.id} className="rounded-2xl border border-border bg-card/80 p-5 shadow-sm"><div className="flex items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-secondary text-muted-foreground"><BellRing className="size-4" /></span><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><h2 className="text-sm font-semibold text-foreground">{rule.name}</h2><button type="button" aria-label={`${rule.enabled ? 'Disable' : 'Enable'} ${rule.name}`} onClick={() => save(rules.map((r) => r.id === rule.id ? { ...r, enabled: !r.enabled } : r))} className={`flex size-7 items-center justify-center rounded-full border ${rule.enabled ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-secondary text-muted-foreground'}`}><Check className="size-3.5" /></button></div><p className="mt-1 text-xs text-muted-foreground">When {rule.event.toLowerCase()}.</p><div className="mt-3 flex items-center justify-between"><span className="inline-flex items-center gap-1.5 text-[11px] text-muted-foreground"><Zap className="size-3" />{rule.enabled ? 'Active' : 'Paused'}</span><button type="button" onClick={() => save(rules.filter((r) => r.id !== rule.id))} className="text-muted-foreground hover:text-destructive" aria-label={`Delete ${rule.name}`}><Trash2 className="size-3.5" /></button></div></div></div></div>)}
    </section>
    {showNew && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-2xl"><div className="flex items-start justify-between"><div><h2 className="text-base font-semibold text-foreground">Create automation rule</h2><p className="mt-1 text-xs text-muted-foreground">Define a signal your team should act on.</p></div><button type="button" onClick={() => setShowNew(false)} className="text-muted-foreground">×</button></div><input autoFocus value={name} onChange={(e) => setName(e.target.value)} placeholder="Rule name" className="mt-4 h-10 w-full rounded-md border border-input bg-input/30 px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring/50" /><input value={event} onChange={(e) => setEvent(e.target.value)} placeholder="Trigger condition" className="mt-3 h-10 w-full rounded-md border border-input bg-input/30 px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring/50" /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setShowNew(false)} className="h-9 rounded-md border border-border px-3 text-sm">Cancel</button><button type="button" onClick={add} disabled={!name.trim()} className="h-9 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground disabled:opacity-50">Create rule</button></div></div></div>}
  </div>
}
