...
export function PipelineBoard() {
  ...
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem('helm.pipeline.deals')
  ...
  const addDeal = () => {
    ...
    if (!stage || !cleanCompany) return

    const deal: Deal = {
      id: `d-new-${Date.now()}`,
      company: cleanCompany,
      initials: cleanCompany.split(/\s+/).map((x) => x[0]).join('').slice(0, 2).toUpperCase(),
      value: value.trim() || '$0',
      owner: 'AV',
      age: '0d',
    }
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {deals.map((stage) => (
          <section key={stage.id} className="flex flex-col rounded-xl border border-border bg-card/60">
            <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-3">
              <div className="flex items-center gap-2"><span className="size-2 rounded-full" style={{ backgroundColor: stage.accent }} /><h2 className="text-sm font-medium text-foreground">{stage.name}</h2><span className="rounded-full bg-secondary px-1.5 text-[11px] tabular-nums text-muted-foreground">{stage.deals.length}</span></div>
              <button type="button" aria-label={`Add deal to ${stage.name}`} onClick={() => setStageForNew(stage.id)} className="flex size-6 items-center justify-center rounded-md text-muted-foreground hover:bg-secondary hover:text-foreground"><Plus className="size-3.5" /></button>
            </div>
            <div className="flex flex-1 flex-col gap-2 p-3">
              {stage.deals.map((deal) => (
                <button key={deal.id} type="button" onClick={() => setSelected(deal)} className="group rounded-lg border border-border bg-card p-3 text-left hover:border-primary/40">
                  <div className="flex items-start justify-between gap-2"><p className="text-sm font-medium text-foreground">{deal.company}</p><span className="shrink-0 text-sm font-semibold tabular-nums text-foreground">{deal.value}</span></div>
                  <div className="mt-3 flex items-center justify-between"><span className="flex size-6 items-center justify-center rounded-full bg-secondary text-[10px] font-semibold text-muted-foreground ring-1 ring-border">{deal.owner}</span><span className="text-xs text-muted-foreground">{deal.age} in stage</span></div>
                </button>
              ))}
            </div>
          </section>
