...
import { LayoutGrid, BarChart3, Users, Database, FileText, Workflow, Settings, LifeBuoy, ChevronsUpDown, Hexagon, User, CreditCard, LogOut, BellRing, Activity, FileBarChart, UserPlus } from 'lucide-react'
import { cn } from '@/lib/utils'
import { DropdownMenu, DropdownMenuContent, DropdownMenuGroup, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from '@/components/ui/dropdown-menu'

const nav = [
  { group: 'Workspace', items: [{ icon: LayoutGrid, label: 'Overview', href: '/' }, { icon: BarChart3, label: 'Analytics', href: '/analytics' }, { icon: Users, label: 'Accounts', href: '/accounts' }, { icon: Workflow, label: 'Pipelines', href: '/pipelines' }, { icon: BellRing, label: 'Alerts & automation', href: '/alerts' }, { icon: Activity, label: 'Activity', href: '/activity' }] },
  { group: 'Data', items: [{ icon: Database, label: 'Databases', href: '/databases' }, { icon: FileText, label: 'Client files', href: '/client-files' }, { icon: FileBarChart, label: 'Reports', href: '/reports' }] },
  { group: 'Workspace setup', items: [{ icon: UserPlus, label: 'Team & access', href: '/team' }] },
]
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'k') {
        ...
        document.querySelector<HTMLInputElement>('input[aria-label="Search accounts and files"]')?.focus()
      }
    }
