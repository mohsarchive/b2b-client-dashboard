...
export function Topbar() {
  ...
  useEffect(() => {
    try { setReadNotifications(window.localStorage.getItem('helm.notifications.read') === '1') } catch {}
  }, [])
  ...
  useEffect(() => {
    const onPointerDown = (event: MouseEvent) => { if (searchRef.current && !searchRef.current.contains(event.target as Node)) setSearchOpen(false) }
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-border bg-background/80 px-5 backdrop-blur-xl">
      <div className="min-w-0"><div className="flex items-center gap-2 text-sm"><span className="text-muted-foreground">Workspace</span><span className="text-muted-foreground/40">/</span><span className="font-medium text-foreground">{current}</span></div></div>
      ...
      <div className="relative">
        ...
        {filterOpen && <div className="absolute right-0 top-11 z-50 w-64 rounded-lg border border-border bg-popover p-3 shadow-xl"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-medium text-foreground">Quick filters</p><button type="button" onClick={() => setFilterOpen(false)} aria-label="Close filters"><X className="size-4 text-muted-foreground" /></button></div><label className="flex cursor-pointer items-center gap-3 rounded-md p-2 hover:bg-accent"><input type="checkbox" checked={onlyRisk} onChange={(e) => setOnlyRisk(e.target.checked)} className="size-4 accent-[var(--primary)]" /><span className="text-sm text-foreground">At-risk accounts only</span></label>{onlyRisk && <button type="button" onClick={() => { router.push('/accounts?status=At%20risk'); setFilterOpen(false) }} className="mt-2 w-full rounded-md bg-primary px-3 py-2 text-xs font-medium text-primary-foreground">View filtered accounts</button>}{onlyRisk && <button type="button" onClick={clearQuickFilter} className="mt-2 w-full rounded-md border border-border px-3 py-2 text-xs text-muted-foreground hover:bg-secondary">Clear filter</button>}</div>}
      </div>
      <div className="relative">
        <button type="button" aria-label="Open notifications" aria-expanded={notificationsOpen} onClick={() => { setNotificationsOpen((open) => !open); setFilterOpen(false) }} className="relative flex size-9 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"><Bell className="size-4" />{!readNotifications && <span className="absolute right-2 top-2 size-1.5 rounded-full bg-primary" />}</button>
        {notificationsOpen && <div className="absolute right-0 top-11 z-50 w-80 rounded-lg border border-border bg-popover p-3 shadow-xl"><div className="mb-2 flex items-center justify-between"><p className="text-sm font-medium text-foreground">Notifications</p><span className="rounded-full bg-primary/10 px-2 py-0.5 text-[10px] text-primary">{readNotifications ? '0 new' : `${notificationItems.length} new`}</span></div>{notificationItems.map((item) => <button key={item.text} type="button" onClick={() => { router.push(item.href); setNotificationsOpen(false) }} className="flex w-full items-start gap-3 rounded-md p-2.5 text-left hover:bg-accent"><span className={cnDot(readNotifications)} /><span className="text-xs text-foreground">{item.text}</span></button>)}<button type="button" onClick={markNotificationsRead} className="mt-1 flex w-full items-center justify-center gap-1.5 rounded-md border border-border py-2 text-xs font-medium text-muted-foreground hover:bg-accent hover:text-foreground"><Check className="size-3.5" /> Mark all as read</button></div>}
      </div>
