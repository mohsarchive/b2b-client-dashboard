...
export function Topbar() {
  ...
  return (
    <header className="premium-enter sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-border bg-background/80 px-5 backdrop-blur-xl">
      ...
      <div className="relative">
        ...
        {filterOpen && <div className="premium-pop absolute right-0 top-11 z-50 w-64 rounded-lg border border-border bg-popover p-3 shadow-xl"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-medium text-foreground">Quick filters</p><button type="button" onClick={() => setFilterOpen(false)} aria-label="Close filters"><X className="size-4 text-muted-foreground" /></button></div><label className="flex cursor-pointer items-center gap-3 rounded-md p-2 hover:bg-accent"><input type="checkbox" checked={onlyRisk} onChange={(e) => setOnlyRisk(e.target.checked)} className="size-4 accent-[var(--primary)]" /><span className="text-sm text-foreground">At-risk accounts only</span></label>{onlyRisk && <button type="button" onClick={() => { router.push('/accounts?status=At%20risk'); setFilterOpen(false) }} className="mt-2 w-full rounded-md bg-primary px-3 py-2 text-xs font-medium text-primary-foreground">View filtered accounts</button>}{onlyRisk && <button type="button" onClick={clearQuickFilter} className="mt-2 w-full rounded-md border border-border px-3 py-2 text-xs text-muted-foreground hover:bg-secondary">Clear filter</button>}</div>}
      </div>
